...
/** <p>
 * A utility class which helps ease integration with Barcode Scanner via {@link Intent}s. This is a simple way to invoke barcode scanning and receive the result, without any need to integrate, modify, or learn the project's
 * source code.
 * </p>
 * 
 ...
 * 
 * {
 * 	&#064;code IntentIntegrator integrator = new IntentIntegrator(yourActivity);
 * 	integrator.initiateScan();
 * }
 * </pre>
 * 
 * <p>
 * Note that {@link #initiateScan()} returns an {@link AlertDialog} which is non-null if the user was prompted to download the application. This lets the calling app potentially manage the dialog. In particular, ideally, the
 * app dismisses the dialog if it's still active in its {@link Activity#onPause()} method.
 * </p>
 * 
 * <p>
 * You can use {@link #setTitle(String)} to customize the title of this download prompt dialog (or, use {@link #setTitleByID(int)} to set the title by string resource ID.) Likewise, the prompt message, and yes/no button
 ...
 * 
 * <p>
 * By default, this will only allow applications that are known to respond to this intent correctly do so. The apps that are allowed to response can be set with {@link #setTargetApplications(Collection)}. For example, set to
 ...
 * 
 * <p>
 * Some formats are not enabled by default even when scanning with {@link #ALL_CODE_TYPES}, such as {@link com.google.zxing.BarcodeFormat#PDF_417}. Use {@link #initiateScan(java.util.Collection)} with a collection containing
 * the names of formats to scan for explicitly, like "PDF_417", to use such formats.
 * </p>
 * 
 * @author Sean Owen
 * @author Fred Lin
